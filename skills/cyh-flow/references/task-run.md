...
Use this mode to independently claim and process stored work from `.cyh-flow/tasks/YYYY-MM-DD.md`. The Markdown task fields are the sole persistent ownership and status authority. Host Task tools or an agent-team board may mirror live progress while the document-backed claim remains authoritative.
...
Every worker receives the pool path, selection boundary, applicable instructions, unique Agent identity, and the same claim-before-acting contract below. Each worker claims for itself, handles its successfully claimed task, records completion or waiting state through the document, and returns the task ID, changed paths, validation, status, and question if blocked. A single selected task uses one worker.
...
The successful atomic claim result is the worker's sole authority to act. `doing` tasks retain their recorded owner until the user explicitly authorizes reassignment; prior unlocked reads and manual status edits confer no ownership.
...
Independent Agents run concurrently after passing the conflict gate and while their claimed tasks retain disjoint file ownership and runtime resources. If a later overlap appears, pause before the next mutation, apply the deterministic priority rule, and move the lower-priority task to `waiting`. Each worker stays within its claimed scope.
