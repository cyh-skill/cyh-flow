...
Use this mode for an explicit, persistent finding-zero objective. Create a durable Goal, discover supported findings through the selected evidence contract, repair them with one writer, rerun every required lane invalidated by the repair, and continue until the completion gate is satisfied. The contract may select review, automated checks, simulator or device flows, browser flows, runtime evidence, security analysis, performance validation, CI, or business acceptance as finding sources.
...
The user owns the required evidence lanes and may define any combination, including ordinary review, `review deep`, a named browser journey, simulator plus Web testing, automated tests, runtime or log checks, security or performance validation, or a custom acceptance procedure. Record that selection verbatim in the Goal contract. Without a named contract, start from source inspection and the smallest risk-appropriate static or automated checks; words such as “recheck” or “finding zero” and the presence of a Web UI do not select review or browser testing. Only selected lanes define completion, and one lane cannot substitute for another.

Top-level `<cyh-flow> auto` delegates derivation of the review and automated-check matrix defined by [auto.md](auto.md). Browser, simulator, device, deployment, remote-environment, and business-acceptance lanes still require explicit selection by the user or an authoritative acceptance contract. Record the resulting matrix as the convergence contract and keep any selected but unavailable lane blocked.
...
Maintain one mutually exclusive repair writer and one coordinator-owned finding ledger. Evidence agents remain read-only against the shared repository; the coordinator verifies, deduplicates, and records their results before the writer repairs them. When the selected lane is `review deep`, load [review-deep.md](review-deep.md) and use its four-specialist-plus-master protocol; ordinary review keeps its single reviewer. Serialize agents that would otherwise operate the same browser target, simulator, device, database, cache, build output, or external environment.
...
3. Run focused rechecks inside the selected evidence lanes after each coherent change and separate introduced failures from the baseline. Extra diagnostics may support understanding or repair without becoming completion lanes; any concrete in-scope defect they prove still enters the ledger.
