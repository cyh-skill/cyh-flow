...
The user may name the validation evidence for this repair, such as a focused test, four-lens review, simulator scenario, browser journey, runtime check, or performance measurement. Treat that request as authoritative and do not substitute an easier lane. If four-lens review is selected, load [review.md](review.md) and apply its live best-effort protocol. If no lane is specified, select and disclose checks proportionate to the changed behavior and risk.
