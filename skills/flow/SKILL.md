...
- Addressable worker mailboxes and follow-up turns are capability-gated, not a cross-host assumption. Codex exposes them directly. Claude Code may expose `SendMessage` and shared team coordination only when the user has already enabled experimental agent teams; use them when available, but never enable that experiment as a Skill side effect. A review run without addressable follow-up delivery finishes all four specialist lanes first, validates their compact artifact manifest, and then starts a fresh master with the target locator plus that manifest; it does not overlap the master's independent target inspection.
...
- Requests to review, re-review, audit a diff, or inspect a PR are `review` and use four independent read-only specialist lanes to discover evidence-backed candidates, followed by a fresh master that independently reads the target and actively tries to falsify every candidate before assigning disposition, repair boundary, or priority. Review discovers and proves problems; it reads relevant tests and existing CI state as context but never runs a full unit, integration, lint, typecheck, build, or end-to-end suite as a completion gate. A reviewer may run only the smallest focused non-mutating reproduction needed to prove or disprove a concrete candidate. Resolve GitHub PRs through authenticated `gh` and run one deterministic preparation pass that caches complete raw responses plus disposable source access for the cycle; this is transport, not an AI summary, lock, stable-target contract, or final drift gate. Specialists and master exchange compact validated file artifacts instead of full reports in agent messages, and deterministic scripts own manifest reconciliation plus idempotent comment delivery and readback. An explicit GitHub PR URL authored by someone other than the authenticated `gh` user has one narrow delivery exception: unless the user opts out, a completed ordinary review automatically posts its final result once as an ordinary PR comment. Treat only the explicit first argument `auto` after `review` as unattended review mode, require a GitHub PR target, establish one Goal, and launch the deterministic `scripts/review_watch.py` watcher so recurring `gh` queries, cursor comparison, retry backoff, and self-comment filtering do not require the model while the PR is quiet. Use the host's longest safe process wait when it must periodically resume to retain the watcher handle. Run the same complete review cycle against the current live PR, publish each verified cycle result through `scripts/review_publish.py`, and resume substantive AI work only when the watcher reports code or human-discussion changes. `review auto` never freezes or locks the PR, never edits its code, and ends successfully only when a current completed cycle is clean with no intervening relevant event or the user explicitly says to end; it never approves, requests changes, resolves threads, or merges.
...
Parallelism must remain conflict-free. Do not give multiple writers overlapping files, generated artifacts, migrations, lockfiles, schemas, or Git state, and do not concurrently mutate the same dependency cache, simulator, device, browser target, database, build output, or external environment. The mode reference defines its writer topology: `plan` and `task-add` use read-only analysis workers plus one document writer; `build` uses disjoint implementation and validation workers; top-level `auto` adopts the build topology first and the converge topology only after the build Goal completes; `review` uses its fixed four specialists and fresh master, overlapping the master's independent target inspection when capacity permits; `fix` and `converge` use one repair writer plus parallel read-only investigation or evidence workers; `task-run` uses independently claiming workers whose tasks and resources do not conflict.
