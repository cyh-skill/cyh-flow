...
- Requests to review, re-review, audit a diff, or inspect a PR are `review` and use four independent read-only specialist lanes to admit only evidence-backed candidates, followed by a fresh master that actively tries to falsify every candidate against one immutable snapshot before assigning disposition, repair boundary, or priority. Resolve GitHub PRs through authenticated `gh`; when the current checkout cannot represent the exact remote SHAs without interference, use a disposable clone instead of treating local dirtiness or an unrelated branch as a blocker. When capacity permits, the fact-only master precheck overlaps the four lanes; later live-target drift is reported but never causes an automatic wait or rerun. An explicit GitHub PR URL authored by someone other than the authenticated `gh` user has one narrow delivery exception: unless the user opts out, a completed review automatically posts its final result once as an ordinary PR comment after final verification; it never approves, requests changes, resolves threads, or merges.
...
Parallelism must remain conflict-free. Do not give multiple writers overlapping files, generated artifacts, migrations, lockfiles, schemas, or Git state, and do not concurrently mutate the same dependency cache, simulator, device, browser target, database, build output, or external environment. The mode reference defines its writer topology: `plan` and `task-add` use read-only analysis workers plus one document writer; `build` uses disjoint implementation and validation workers; top-level `auto` adopts the build topology first and the converge topology only after the build Goal completes; `review` uses its fixed four specialists and fresh master, overlapping the master's snapshot precheck when capacity permits; `fix` and `converge` use one repair writer plus parallel read-only investigation or evidence workers; `task-run` uses independently claiming workers whose tasks and resources do not conflict.
