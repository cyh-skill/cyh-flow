...
- Requests to review, re-review, audit a diff, or inspect a PR are `review` and use four independent read-only specialist lanes plus a fresh master recheck against one immutable snapshot. When capacity permits, the master precheck overlaps the four lanes; later live-target drift is reported but never causes an automatic wait or rerun.
