...
Treat only the explicit first argument `auto` after `build` as auto mode. Do not infer auto mode merely because the request is large, asks for speed, or contains several tasks.
