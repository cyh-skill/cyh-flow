...
Receive the immutable review packet first and exactly four lane records from `codex-correctness`, `ponytail-complexity`, `differential-security`, and `performance-engineer` before finalizing. A record may be a valid specialist envelope or an explicit terminal transport record. When started concurrently, invoke deterministic packet verification once, independently inspect the frozen snapshot, and establish a requirement baseline, scope-decision map, ownership map, impact closure, changed behavior, and known unknowns before reading any specialist claim. Then use the runtime's mailbox wait mechanism until the coordinator supplies all four records; do not issue a provisional verdict or return early.
