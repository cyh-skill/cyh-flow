...
Resolve the exact repository, objective, source of truth, scope, non-goals, starting target, and stopping condition. Then use the available Codex Goal mechanism to create one durable objective. Do not claim that a Goal exists when none was created, assign a token budget unless the user explicitly requested one, replace an unfinished different Goal, or mark completion merely because work stopped.
