...
Use event-driven mailbox waits with a minutes-scale timeout rather than repeated short waits or status polling. Query the agent list only to diagnose a missing or stuck result. The master is not a fifth specialist vote or a summary writer: after all four records arrive, it ignores proposed severity and repair advice, assumes each candidate may be wrong, and actively searches for counterexamples in the baseline, state producers, entry points, authoritative cross-boundary contracts, role combinations, historical scope decisions, alternate paths, tests, and runtime evidence. Only after the candidate survives falsification may the master select a disposition, repair boundary, and P0-P3 priority.
