...
The four specialists plus fresh master are this mode's required subagent decomposition. Use maximum available concurrency for all useful work. When at least five subagent slots and addressable follow-up mailboxes are available, overlap the master's independent packet precheck with the four lanes, but never let it finalize before all four reach a terminal result. On any run without enabled persistent mailbox delivery, finish the four lanes first and then start a fresh master with their terminal records. Do not replace them with coordinator-only review, collapse roles, or add a writing agent.
...
2. For a GitHub PR, treat authenticated `gh` and the immutable commit SHAs it returns as the remote source of truth. Resolve the explicit PR URL or `OWNER/REPO#NUMBER` with `gh pr view <number-or-url> --repo <owner/repo> --json url,number,title,body,baseRefName,baseRefOid,headRefName,headRefOid,isCrossRepository,files,commits,reviews,statusCheckRollup`; use `gh pr diff <number-or-url> --repo <owner/repo> --patch` when immediate patch evidence is useful, and use `gh api` only for required GitHub data not exposed by those commands. Do not open a browser for ordinary PR metadata or source inspection, and do not review a stale local approximation when the remote head differs.
3. A dirty checkout, unrelated local branch, missing local PR branch, or another repository already occupying the workspace is a routing signal, not a review blocker. Never switch, reset, clean, or run `gh pr checkout` in the user's current repository merely to inspect a PR, and do not ask whether the current checkout may be reused when the remote target is already explicit.
4. If an existing repository already contains the exact base and head objects, freeze those SHAs without checking either one out. Otherwise create one system temporary directory, use `gh repo clone <owner/repo> <temporary>/repo -- --filter=blob:none --no-checkout`, fetch the base and `refs/pull/<number>/head` into disposable refs, verify that both resolved object IDs equal the values returned by `gh pr view`, and run the snapshot script against that disposable repository. For a fork PR, the base repository's `refs/pull/<number>/head` is preferred because it resolves the reviewed head without modifying or depending on the contributor's checkout. Read repository instructions and surrounding code from the frozen snapshot; `gh pr diff` alone is not enough for a full review.
5. Reuse one matching disposable clone for all lanes and shared checks in the run. Do not create one clone per reviewer or download files one by one through the API. Use CodeGraph only when its index is known to match the frozen repository and head; otherwise inspect the frozen snapshot with repository-native search and record the unavailable index as a coverage detail, not a target-resolution failure.
6. Include staged, unstaged, and relevant untracked source files for local reviews. Validate that the diff is non-empty before drawing conclusions.
7. Establish the originating requirement, issue, plan, or user intent. If none exists, state that requirement completeness cannot be verified.
...
Build the human-readable packet around the script output and record fact-only requirement context with provenance, explicit accepted or deferred scope decisions, base, head, merge-base, changed files, project constraints, an initial impact seed, known unknowns, packet directory, snapshot root, and allowed checks. Do not put provisional findings, severity, confidence, or repair advice in the shared packet. Reviewers inspect the frozen snapshot and retained artifacts, never the mutable source checkout. A review result belongs to that immutable packet, not to a branch or PR name forever.
...
Each lane reads the full diff, relevant surrounding code, applicable instructions, and the impact-map surfaces material to its lens. It treats suspicious code as a hypothesis and admits a candidate only after satisfying the common contract's introduction, reachability, authoritative-contract, scope-decision, and repair-ownership gates. In particular, a condition or enum does not prove a producible business state, a frontend action gate does not prove authorization semantics, and a nearby historical implementation does not override an explicit deferred decision. Put any plausible item with a required unknown gate in `open_questions` without severity or repair advice. Minimize model/tool round trips without narrowing coverage: read the packet, contract, role, and deterministic verification result in one first-stage batch; use `rg` or CodeGraph to form the next candidate set; batch every independent file read, history query, and check within each bounded stage; never reread an unchanged artifact merely to regain context. Use adaptive sequential investigation only when one result genuinely determines the next query. Do not reconstruct hashes or compare against the mutable source checkout when `scripts/review_snapshot.py verify` already validated the retained packet.
...
When at least five subagent slots and addressable follow-up delivery are available, start a fresh read-only master with no inherited conversation turns at the same time as the four specialists using [review/master-reviewer.md](review/master-reviewer.md). Initially give it only the immutable packet, deterministic verification result, retained shared-check evidence, master role, and frozen snapshot access. Before seeing specialist claims, it independently establishes and retains the requirement baseline, scope decisions, architecture ownership, impact closure, and known unknowns, then waits. Send each valid envelope or terminal transport record to the master as it arrives; the master must receive exactly one terminal record for each required role and cannot finalize early. Otherwise prioritize the four specialists, collect exactly one terminal record from each, and only then start a fresh master with the packet, shared evidence, and all four records.

When the host provides event-driven mailbox waits, use a minutes-scale timeout rather than repeated short waits or status polling, and query the agent list only to diagnose a missing or stuck result. Do not invent that mechanism on hosts without it. The master is not a fifth specialist vote or a summary writer: after all four records arrive, it ignores proposed severity and repair advice, assumes each candidate may be wrong, and actively searches for counterexamples in the baseline, state producers, entry points, authoritative cross-boundary contracts, role combinations, historical scope decisions, alternate paths, tests, and runtime evidence. Only after the candidate survives falsification may the master select a disposition, repair boundary, and P0-P3 priority.

Agreement is not proof and disagreement is not disproof; `reported_by` is provenance only and must never raise confidence or severity. A state-dependent candidate without a proven state producer is unresolved. An authorization or cross-boundary candidate without the authoritative enforcement contract is unresolved, or is narrowed to the independently proven local behavior without claiming the unverified impact. A supported issue covered by an explicit accepted deferral is `known_deferred`, not a current actionable finding. Keep disproved or unsupported candidates in the rejected or unresolved audit trail instead of silently deleting them. The coordinator must validate the master's target fingerprint and schema, verify that every input candidate and open question has exactly one disposition, reject any verified finding whose applicable evidence gate is not passed or whose falsification is inconclusive, and recompute the clean gate from the four lane records, impact closure, evidence lanes, coverage, and adjudicated candidates. Never trust `clean_eligible` as an assertion by itself.
...
Report a clean result only when all four specialist lanes and the master completed against the same frozen target, the impact map has no unexplained difference, no verified finding, advisory, or unresolved actionable candidate remains, no material coverage item is unverified, and every applicable evidence lane was exercised. A properly evidenced `known_deferred` item does not block a clean result for the explicitly reviewed scope, but must be disclosed separately with its decision provenance and revisit condition. Phrase a clean result as “no supported actionable findings within the reviewed target, enumerated impact surface, and executed evidence lanes,” then list known deferred items and immaterial unverified areas separately. A blocked lane, corrupted or mismatched packet, missing report, required surface that cannot be inspected, or unresolved material candidate makes the result incomplete rather than clean.
