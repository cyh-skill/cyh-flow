...
cyh-flow 的契约是仅显式调用：普通的自然语言 plan、build、auto、review、fix、converge、task-add 或 task-run 请求不应自动进入这套流程。Codex 需要用户输入 `$cyh-flow ...` 或主动选择该 Skill，并由 `skills/cyh-flow/agents/openai.yaml` 强制执行；Claude Code 插件加载同一个 `skills/cyh-flow/SKILL.md`，用户以 `/cyh-flow:cyh-flow ...` 调用。显式调用后，一次性修复明确问题属于 `fix`；只有明确要求跨轮复查和修复、模拟器或 Web 验收循环，或“直到 finding 归零”时才进入 `converge`；只有顶层 `cyh-flow auto` 会把 `build auto` 与全证据 `converge` 串起来。若一句话混合了多个模式，cyh-flow 会保留每个阶段的权限边界：方案不会自动进入实现，审查不会自动变成修复，task-add 不会自动处理刚收录的事项，任何本地实现或修复也不会自动进入交付。
