...
面向 Codex 的跨项目软件交付工作流 Skill，用同一套授权边界处理方案设计、代码实现、只读审查、一次性修复、Goal 驱动的持续收敛，以及带截图证据的长期任务池，避免“先看看”被误解为直接改代码，也避免一次实现请求被自动扩大成提交、推送或部署。
...
| 模式 | 适合处理 | 默认允许 | 默认不允许 |
| --- | --- | --- | --- |
| `plan` | 统一定义需求行为、实现方式和验收路径 | 读取代码、配置、文档和实时状态；创建或更新一份同时承担 Spec 与 Plan 职责的需求 `.md` | 修改应用代码或无关文档、把同一需求拆成多份决策文档、建分支、提交、推送、部署或更新外部系统 |
...
| `fix` | 一次性修复明确 bug、失败行为或 Review finding | 范围内本地修复和与风险相称的针对性验证 | 自动扩大为长期 Goal、未经明确要求的提交、推送、PR、部署或外部写入 |
| `converge` | 持续发现、修复并复查问题，直到目标范围内 finding 归零 | 建立 Goal、维护 finding 台账，并按用户定义运行 Review、自动化测试、模拟器/真机、Web、运行时、安全或性能证据通道 | 未经明确要求的提交、推送、PR、部署、生产写入或对外消息 |
| `task-add` | 批量收录 bug、小改动、杂项或后续工作 | 只读分析并写入 `.cyh-flow/tasks/YYYY-MM-DD.md`，原样保留用户截图 | 修改应用代码、领取或处理任务、提交、推送或外部写入 |
| `task-run` | 多 Agent 独立领取并处理任务池 | 以 Markdown 状态为准原子领取不同任务，完成本地修改和相应验证 | 抢占 `doing` 任务、绕过领取直接处理、未经明确要求的交付或外部写入 |

七种模式共享几条硬边界：永不 Merge PR；不擅自丢弃或覆盖用户工作；破坏性 Git 操作必须先说明精确目标和损失范围；源码检查、自动化测试、CI、部署运行时、UI/设备证据与业务验收分别报告，不能互相替代。
...
$cyh-flow task-add 把下面这批 bug 和小改动分析后收进任务池，并保留我附的截图

$cyh-flow task-run 并发处理任务池里所有可执行事项，有问题集中问我
```

CYH Flow 被设置为仅显式调用：普通的自然语言 plan、build、review、fix、converge、task-add 或 task-run 请求不会自动加载该 Skill，只有用户输入 `$cyh-flow ...`，或在 Skill 选择器中主动选择它时才会进入这套流程。显式调用后，一次性修复明确问题属于 `fix`；只有明确要求 Goal、跨轮复查和修复、模拟器或 Web 验收循环，或“直到 finding 归零”时才进入 `converge`。若一句话混合了多个模式，CYH Flow 会保留每个阶段的权限边界：方案不会自动进入实现，审查不会自动变成修复，task-add 不会自动处理刚收录的事项，任何本地实现或修复也不会自动进入交付。
...
任务池是独立流水线：`task-add` 把一批 bug、小改动或其他事项分析后写入首次收录日期对应的 Markdown，截图原样保存在 `.cyh-flow/tasks/assets/<TASK-ID>/` 并以内嵌相对链接展示；`task-run` 不依赖主 Agent，每个 Agent 都用唯一身份对任务文档执行一次极短的加锁、重读和状态更新，只有把 `pending` 成功写成 `doing` 并留下领取人和时间后才能动手。锁不保存任何任务信息，Markdown 始终是状态和归属的唯一事实来源；并发竞争失败的 Agent 会领取下一条，`doing` 任务不会被自动抢占，完成后改成 `done`，需要用户决定时改成 `waiting` 并保留问题和处理历史。
...
`SKILL.md` 每次先路由到一个主模式 reference，避免把无关流程同时载入上下文；只有 `fix` 或 `converge` 的用户证据通道明确要求四路 Review 时，才继续按需读取 Review 协议。调整模式规则时，应把共享约束留在 `SKILL.md`，把仅属于单一模式的细节留在对应 reference，并使用 Codex 自带的 `skill-creator` 校验 Skill 结构。
