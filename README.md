...
| `review` | 审查工作区、分支、提交或 Pull Request，判断是否以最优实现覆盖完整关联面 | 用确定性脚本冻结不可变目标，四路独立收集满足准入门槛的候选证据，并在槽位允许时同步运行事实型主复查预检；主审逐条尝试证伪后才定结论，实时目标后来变化时立即交付旧快照结论并标记漂移 | 把静态怀疑直接升级为 finding、用 reviewer 数量提高置信度、直接修复、自动等待工作树稳定或重跑、提交、推送、批准、发评论或改变 PR 状态 |
