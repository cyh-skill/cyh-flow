...
| `review` | 审查工作区、分支、提交或 Pull Request，判断是否以最优实现覆盖完整关联面 | GitHub PR 先用已认证 `gh` 解析精确远端 SHA，本地 checkout 不适合时自动改用系统临时目录中的隔离 clone；随后用确定性脚本冻结目标，四路收集满足准入门槛的候选证据，主审逐条尝试证伪后才定结论 | 因本地脏工作树或无关分支阻塞远端 PR Review、为了 Review 切换当前 checkout、把静态怀疑直接升级为 finding、用 reviewer 数量提高置信度、直接修复、自动等待或重跑、提交、推送、批准、发评论或改变 PR 状态 |
